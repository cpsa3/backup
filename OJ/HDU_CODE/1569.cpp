...
////Nickname: ___简言
////Run ID: 
////Submit time: 2011-04-29 18:57:47
////Compiler: Visual C++
//////////////////////////////////////////////////////
////Problem ID: 1569
////Problem Title: 
...
void Init()
{
	...
	scr=0;sink=N*M*2+1;vn=sink+1;sum=0;
	for(i=1;i<=M;i++)
	{
		for(j=1;j<=N;j++)
	...
	for(i=1;i<=M;i++)
	{
		...
		{
			id=(i-1)*N+j;
			Insert(id,id+N*M,map[i][j]);
			if((i+j)&1)
			Insert(scr,id,INF);
			else
			Insert(id+N*M,sink,INF);
			if(i-1>=1)
				Insert(id+N*M,id-N,INF);
			if(i+1<=M)
				Insert(id+N*M,id+N,INF);
			if(j-1>=1)
				Insert(id+N*M,id-1,INF);
			if(j+1<=N)
				Insert(id+N*M,id+1,INF);
		}
	}
