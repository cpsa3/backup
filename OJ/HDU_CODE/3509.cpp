...
////Nickname: ___简言
////Run ID: 
////Submit time: 2010-12-05 15:35:53
////Compiler: Visual C++
//////////////////////////////////////////////////////
////Problem ID: 3509
////Problem Title: 
...
void init()
{
	...
	for(i=2;i<=K;i++)//运用二项式定理
		...
		{
			for(j=1;j<=K+2-i;j++)
				E.v[i][j]=C.v[K-j+1][i-1]*mypow(a,K-i-j+2)%MOD*mypow(b,i-1)%MOD;
		}
}
int main()
{
	...
	while(T--)
	{
		...
		{printf("%I64d\n",s1);continue;}
		//if(N==2)
		//{printf("%I64d\n",mypow(f1,K)*mypow(f2,K)%MOD);continue;}
		init();
		A=matrix_mi(E,N-1);
		__int64 ans[55];
		ans[0]=s1;
		for(i=1;i<=K+1;i++)
			ans[i]=mypow(f2,K-i+1)%MOD*mypow(f1,i-1)%MOD;
		__int64 ANS=0;
		for(i=0;i<=K+1;i++)
...
//http://www.cppblog.com/Yuan/archive/2010/09/04/125891.html